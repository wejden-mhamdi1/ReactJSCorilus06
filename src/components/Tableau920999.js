...
function Tableau920999() {
  ...
  function getErreur(){
   ...
   function getMessagesHeader(){
    ...
    function getRecordAllList(){
      axios.get(`http://localhost:8089/zone/getAllZone`) // Remplacez '/students' par l'URL de votre endpoint pour récupérer les étudiants
      .then(response => {
...
function checkRecordAndError(AllRecords, erreur) {
  const zoneSubstring = AllRecords.nom_zone.substring(4, 7);
  const errorSubstring = erreur.errorNature.substring(2, 5);
  
  if (zoneSubstring === errorSubstring) {
    return AllRecords.data;
  }
  
  return null; // Si la condition n'est pas vérifiée, retourne null ou une autre valeur par défaut
}
        ...
        {fotters95.map((footer, index) => (
          <div key={index}>
            {footer.content.map(record => {
              ...
              return (
                <TableCell style={{ width: '200px' }} key={record.id}>
